...
<p class="text-sm text-surface-500 mb-3">File: <code class="text-surface-300">{cfgPath}</code></p>
{#if error}<div class="text-red-400 mb-2">{error}</div>{/if}
{#if msg}<div class="text-accent mb-2">{msg}</div>{/if}

<textarea class="input font-mono text-xs" rows="28" bind:value={contents} disabled={loading}></textarea>
<div class="mt-3">
...
<p class="text-xs text-surface-500 mt-3">
	Changes take effect on next map load (or <code class="text-surface-300">exec server.cfg</code> via RCON).
</p>
