...
install_urt_service_scaffolding() {
    ...
    #    DropIn directory /etc/systemd/system/urt@%i.service.d/ (one dir per
    #    instance) generated by the UI wizard via the client bot. A per-instance
    #    directory is required: a single shared urt@.service.d/ would apply
    #    every override to every instance (multiple ExecStart= -> bad-setting).
    cat > /etc/systemd/system/urt@.service << 'URTEOF'
[Unit]
Description=Urban Terror 4.3 Dedicated Server (%i)
After=network.target
# Instance-specific environment comes from the per-instance drop-in written
# by R3: /etc/systemd/system/urt@%i.service.d/override.conf
ConditionPathExists=/etc/systemd/system/urt@%i.service.d/override.conf
...
URTEOF
    ...
    #    Alias names are uppercased username-scoped to avoid Cmnd_Alias
    #    collisions when multiple users have urt sudoers on the same host.
    local ALIAS_SUFFIX
    ALIAS_SUFFIX=$(echo "${REAL_USER}" | tr '[:lower:]' '[:upper:]' | tr -cd 'A-Z0-9_')
...
Cmnd_Alias R3_URT_SYSTEMCTL_${ALIAS_SUFFIX} = /bin/systemctl start urt@*, \\
...
Cmnd_Alias R3_URT_DROPIN_${ALIAS_SUFFIX} = /usr/bin/install -d -m 0755 /etc/systemd/system/urt@*.service.d, \\
                           /bin/install -d -m 0755 /etc/systemd/system/urt@*.service.d, \\
                           /usr/bin/tee /etc/systemd/system/urt@*.service.d/*.conf, \\
                           ...
                           /bin/rm -rf /etc/systemd/system/urt@*.service.d, \\
                           /usr/bin/rm -rf /etc/systemd/system/urt@*.service.d

Cmnd_Alias R3_URT_SELF_UNINSTALL_${ALIAS_SUFFIX} = /usr/bin/systemd-run --no-block --collect --unit r3-self-uninstall-*.service /bin/bash -c *, \\
                                   /bin/systemd-run --no-block --collect --unit r3-self-uninstall-*.service /bin/bash -c *

${REAL_USER} ALL=(root) NOPASSWD: R3_URT_SYSTEMCTL_${ALIAS_SUFFIX}, R3_URT_DROPIN_${ALIAS_SUFFIX}, R3_URT_SELF_UNINSTALL_${ALIAS_SUFFIX}
SUDOEOF
