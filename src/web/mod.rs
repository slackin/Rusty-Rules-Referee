...
pub fn build_router(state: AppState) -> Router {
    let api = Router::new()
        ...
        // Map config global defaults (master-only template)
        .route("/map-config-defaults", get(api::mapconfigs::list_map_config_defaults))
        .route("/map-config-defaults/:map_name", get(api::mapconfigs::get_map_config_default))
        .route("/map-config-defaults/:map_name", put(api::mapconfigs::save_map_config_default_endpoint))
        .route("/map-config-defaults/:map_name", delete(api::mapconfigs::delete_map_config_default_endpoint))
        .route("/map-config-defaults/:map_name/propagate", post(api::mapconfigs::propagate_map_config_default))
        ...
        // Player Groups — cross-server shared permission collections
        .route("/player-groups", get(api::player_groups::list_groups))
        .route("/player-groups", post(api::player_groups::create_group))
        .route("/player-groups/:id", get(api::player_groups::get_group))
        .route("/player-groups/:id", put(api::player_groups::update_group))
        .route("/player-groups/:id", delete(api::player_groups::delete_group))
        .route("/player-groups/:id/members", get(api::player_groups::list_members))
        .route("/player-groups/:id/members", post(api::player_groups::add_member))
        .route("/player-groups/:id/members/:guid", put(api::player_groups::update_member))
        .route("/player-groups/:id/members/:guid", delete(api::player_groups::delete_member))
