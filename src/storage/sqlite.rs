...
impl Storage for SqliteStorage {
    ...
    async fn get_servers(&self) -> Result<Vec<GameServer>, StorageError> {
        ...
        Ok(rows.iter().map(|r| GameServer {
            ...
            update_enabled: r.try_get::<i64, _>("update_enabled").map(|v| v != 0).unwrap_or(true),
            created_at: r.get("created_at"),
            updated_at: r.get("updated_at"),
    ...
    async fn get_server(&self, server_id: i64) -> Result<GameServer, StorageError> {
        ...
        Ok(GameServer {
            ...
            update_enabled: row.try_get::<i64, _>("update_enabled").map(|v| v != 0).unwrap_or(true),
    ...
    async fn save_server(&self, server: &GameServer) -> Result<i64, StorageError> {
        if server.id > 0 {
            ...
            .bind(server.update_enabled as i64)
            .bind(server.hub_id)
            .bind(&server.slug)
            .bind(server.id)
            .execute(&self.pool)
    ...
    async fn set_server_update_enabled(&self, server_id: i64, enabled: bool) -> Result<(), StorageError> {
        sqlx::query(
            "UPDATE servers SET update_enabled = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?"
        )
        .bind(enabled as i64)
        .bind(server_id)
        .execute(&self.pool)
        ...
        if hub.id > 0 {
            sqlx::query(
                "UPDATE hubs SET name = ?, address = ?, status = ?, last_seen = ?, \
                 ...
                 update_interval = ?, update_enabled = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?"
            )
            .bind(&hub.name)
            .bind(&hub.address)
            .bind(&hub.status)
            ...
            .bind(hub.update_enabled as i64)
            .bind(hub.id)
            .execute(&self.pool)
            .await
            .map_err(|e| StorageError::QueryFailed(e.to_string()))?;
    ...
    async fn set_hub_update_enabled(&self, hub_id: i64, enabled: bool) -> Result<(), StorageError> {
        sqlx::query(
            "UPDATE hubs SET update_enabled = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?"
        )
        .bind(enabled as i64)
        .bind(hub_id)
        .execute(&self.pool)
